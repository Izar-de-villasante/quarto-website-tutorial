...
I am a {{< var job.position >}} currently working at {{< var job.webpage >}} in {{< var job.city >}}.
...
m <- leaflet() %>%
  addTiles(urlTemplate = "http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png") %>%  # Add default OpenStreetMap map tiles

  addAwesomeMarkers(lng=0.525847725869538, lat=40.81235294636711, popup="IISPV (PO&B)",clusterOptions = markerClusterOptions())%>%
  addMarkers(lng=2.1058071325229624, lat=41.346492855726325, popup="IDIBELL (Brainvitge)",clusterOptions = markerClusterOptions())%>%
  addMarkers(lng=2.239724437547112, lat=41.48540089172277, popup="ICO (UNAC)",clusterOptions = markerClusterOptions())%>%
  addMarkers(lng=2.239724437547112, lat=41.48540089172277, popup="IJC (BIT)",clusterOptions = markerClusterOptions(showCoverageOnHover = T,spiderfyOnMaxZoom = T))%>%
  
  addRectangles(
    lng1 = 2.2434, lat1 = 41.4895,
    lng2 = 2.2362, lat2 = 41.4815,
...
You can check out our units` webpage here: [And Bioinformatics Unit (BIT) where I currently work here](https://www.carrerasresearch.org/es/unidad-de-bioinform%C3%A1tica_130420)
If you are interested on my publications you can check them here:

Or follow me on my youtube channel with tutorials.
[Youtube channel]()
You cacn also clone and modify this webpage if you want by branching the github repo.
